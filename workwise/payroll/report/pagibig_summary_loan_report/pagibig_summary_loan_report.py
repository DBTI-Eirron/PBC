...
def get_data(filters):
	...
	if not employee_list:
		...
		for emp in employee_list:
			HDMF_amount = flt(HDMF_map.get(emp.name, {}).get("HDMFL"))
			HDMFE_amount = 0.00
			row = {
				"hdmf_no": emp.hdmf_no,
				...
				"HDMF": format_precision(HDMF_amount, filters.value_precision),
				"HDMFE": format_precision(HDMFE_amount, filters.value_precision),
				"tin": emp.tin,
...
def get_employees(filters):
	...
	if not "Administrator" in frappe.get_roles(cur_user):
		employees = frappe.db.sql(""" SELECT DISTINCT TE.`name`, TE.hdmf_no, PR.`employee`, UPPER(TE.last_name) as last_name, UPPER(TE.first_name) as first_name, UPPER(TE.middle_name) as middle_name, TE.`tin`, TE.`birthday`
			FROM `tabPayroll Register` PR INNER JOIN `tabEmployee` TE ON PR.employee = TE.`name` INNER JOIN `tabPayroll Register Entries` PRE ON PR.`name` = PRE.`parent` 
