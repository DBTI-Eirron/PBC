...
def execute(filters=None):
	# YTD register Report (monthly)
	if not filters: filters = frappe._dict({})
	validate_filters(filters)

	month_range = monthrange(int(filters.payroll_year), int(filters.payroll_month))
	from_date = datetime.date(int(filters.payroll_year),int(filters.payroll_month),month_range[0])
	to_date = datetime.date(int(filters.payroll_year),int(filters.payroll_month),month_range[1])
