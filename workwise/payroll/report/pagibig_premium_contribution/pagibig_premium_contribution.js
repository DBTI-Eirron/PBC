			...
			callback: function(r) {
				if(r.message){
					...
					report.page.add_inner_button(__("Print PagIbig Certificate"), function() {
						...
						window.open("http://"+host_link+"/jasperserver/flow.html?_flowId=viewReportFlow&_flowId=viewReportFlow&ParentFolderUri=%2FReports&reportUnit=%2FReports%2Fhdmf_certificate&standAlone=true&j_username="+username+"&j_password="+password+"&output=pdf&company="+company+"&from_date="+from_date+"&to_date="+to_date+"&group="+group+"&user="+session_user);
					});
					report.page.add_inner_button(__("Print PagIbig Premium Contribution"), function() {
						...
						var company = frappe.query_report_filters_by_name.company.get_value();
						var group = frappe.query_report_filters_by_name.period_group.get_value();
						window.open("http://"+host_link+"/jasperserver/flow.html?_flowId=viewReportFlow&_flowId=viewReportFlow&ParentFolderUri=%2FReports&reportUnit=%2FReports%2Fpagibig_premium_contribution&standAlone=true&j_username="+username+"&j_password="+password+"&output=pdf&company="+company+"&from_date="+from_date+"&to_date="+to_date+"&group="+group+"&user="+session_user);
					});
					report.page.add_inner_button(__("Print PagIbig Contribution Collection"), function() {
						...
						var company = frappe.query_report_filters_by_name.company.get_value();
						var group = frappe.query_report_filters_by_name.period_group.get_value();
						window.open("http://"+host_link+"/jasperserver/flow.html?_flowId=viewReportFlow&_flowId=viewReportFlow&ParentFolderUri=%2FReports&reportUnit=%2FReports%2Fpagibig_contribution_collection&standAlone=true&j_username="+username+"&j_password="+password+"&output=pdf&company="+company+"&from_date="+from_date+"&to_date="+to_date+"&group="+group+"&user="+session_user);
					});
