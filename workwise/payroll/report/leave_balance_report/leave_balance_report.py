...
def get_data(filters):
	...
	if filters.from_date and filters.to_date and not filters.year:
		from_date = filters.from_date
		to_date = filters.to_date

	employees = get_employees(filters)
	for emp in employees:
		leave_balance = frappe.db.sql(""" SELECT DISTINCT `employee_name`,`from_date`,`name`,`leave_type`,`credits`,`used_credits`,`credits`-`used_credits` as balance FROM `tabLeave Balance` WHERE `employee`=%s AND `to_date` <= %s """, (emp.name, to_date),as_dict=True)
