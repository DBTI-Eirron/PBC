...
def execute(filters=None):
	...
	for emp in employee_list:
		...
		for p in periods:
			...
			total_basis += period_amount
			row.append(period_amount)
		bonus_pay = total_basis / 12
		row += [total_basis, bonus_pay]
		data.append(row)
...
def get_employees(filters):
	employees = frappe.db.sql("""SELECT `name`, full_name
	 	FROM tabEmployee
...
def get_period_map(filters, employee_list, from_date, to_date):
	period_details = frappe.db.sql(""" SELECT PR.employee, PR.posting_date, PR.bonus, PR.period
	...
	for d in period_details:
		...
		if period_map[d.employee][d.period]:
			period_map[d.employee][d.period] += flt(d.bonus, 2)
		else:
			period_map[d.employee][d.period] = flt(d.bonus, 2)
