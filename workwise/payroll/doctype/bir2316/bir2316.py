...
class BIR2316(Document):
	...
	def get_current_bir(self):
		...
		for e in emp:
			...
			self.get_nontax_income(e, entry)
			self.get_tax_income(e, entry)
			self.get_summary(e, entry)
	...
	def set_employee_information(self, e, entry):
		...
		employee_address = frappe.db.sql(""" SELECT DISTINCT TA.`address_line1`, TA.`pincode`, TA.`address_type` FROM `tabDynamic Link` DL JOIN `tabAddress` TA ON DL.`parent`=TA.`name` 
			WHERE DL.`parenttype` = "Address" AND DL.`link_doctype` = "Employee" AND DL.`parent` = TA.`name` 
			AND TA.`address_type` = "Foreign" OR TA.`address_type` = "Local Home" OR TA.`address_type` = "Registered" 
			AND DL.`link_name` = %s """, (self.employee), as_dict=1)
		...
		employee_contact = frappe.db.sql(""" SELECT DISTINCT TC.`phone` FROM `tabContact` TC JOIN `tabDynamic Link` DL 
			WHERE DL.`parenttype` = "Contact" AND DL.`link_doctype` = "Employee" 
			AND DL.`link_name` = %s LIMIT 1""", (self.employee), as_dict=1)
	...
	def set_qualified_dependent_children(self, e, entry):
		...
		i = 1
		employee_family = frappe.db.sql(""" SELECT DISTINCT `full_name`, `birthday` FROM `tabFamily Members` 
			WHERE `is_qualified_dependent` = 1 AND `parent` = %s """, (self.employee), as_dict=1)
	...
	def set_employer_information_present(self, e, entry):
		...
		company_address = frappe.db.sql(""" SELECT DISTINCT TA.`address_line1`, TA.`pincode`, TA.`address_type` 
			FROM `tabDynamic Link` DL JOIN `tabAddress` TA WHERE DL.`parenttype` = "Address" AND DL.`link_doctype` = "Company" AND DL.`parent` = TA.`name` AND TA.`address_type` = "Registered" AND DL.`link_name` = %s """, (e.company), as_dict=1)
	...
	#GET NON-TAXABLE/EXEMPT COMPENSATION INCOME
	def get_nontax_income(self, e, entry):
		...
		self.get_ntax_bs(e, entry, tr_map, salary)
		self.get_ntax_ho(e, entry, tr_map, salary)
		self.get_ntax_nd(e, entry, tr_map, salary)
		self.get_ntax_ot(e, entry, tr_map, salary)
		self.get_ntax_bonus_and_tax_bonus(e, entry, tr_map)
		self.get_ntax_demi(e, entry, tr_map, salary)
		self.get_ntax_contrib(e, entry, tr_map, salary)
		self.get_ntax_other(e, entry, tr_map, salary)
		self.get_ntax_hazard(e, entry, tr_map, salary)

		#SET NON-TAXABLE/EXEMPT COMPENSATION INCOME
	...
	def get_ntax_bs(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			...
			if _type != "None":
				if bir_type == "Basic" and is_taxable == 0:
					entry['ntax_bs'] += d.amount

		return entry

	def get_ntax_ho(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Holiday" and is_taxable == 0:
					entry['ntax_ho'] += d.amount

		return entry

	def get_ntax_nd(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Night Differential" and is_taxable == 0:
					entry['ntax_nd'] += d.amount

		return entry

	def get_ntax_ot(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Overtime" and is_taxable == 0:
					entry['ntax_ot'] += d.amount

		return entry

	def get_ntax_bonus_and_tax_bonus(self, e, entry, tr_map):
	...
	def get_ntax_demi(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Deminimis" and is_taxable == 0:
					entry['ntax_demi'] += d.amount

		return entry

	def get_ntax_contrib(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Contribution" and _type == "Deduction":
					entry['ntax_contrib'] += d.amount

				if bir_type == "Contribution" and _type == "Income":
					entry['ntax_contrib'] -= d.amount

		return entry

	def get_ntax_other(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Other" and is_taxable == 0:
					entry['ntax_other'] += d.amount

		return entry

	def get_ntax_hazard(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Hazard" and is_taxable == 0:
					entry['ntax_hazard'] += d.amount

		return entry
	
	#GET TAXABLE COMPENSATION INCOME
	def get_tax_income(self, e, entry):
		tr_map = get_transaction_map()
		salary = frappe.db.sql("""SELECT pr.employee, pr.employee_name, pre.pay_code, pre.amount FROM `tabPayroll Register` pr
			INNER JOIN `tabPayroll Register Entries` pre ON pre.parent = pr.`name`
			WHERE employee = %s AND pr.posting_date >= %s AND pr.posting_date <= %s """,(e.name, self.from_date, self.to_date), as_dict=True)

		self.get_tax_bs(e, entry, tr_map, salary)
		self.get_tax_rep(e, entry, tr_map, salary)
		self.get_tax_transpo(e, entry, tr_map, salary)
		self.get_tax_cola(e, entry, tr_map, salary)
		self.get_tax_housing(e, entry, tr_map, salary)
		self.get_other_reg_a(e, entry, tr_map, salary)
		self.get_other_reg_b(e, entry, tr_map, salary)

		self.get_tax_commission(e, entry, tr_map, salary)
		self.get_tax_sharing(e, entry, tr_map, salary)
		self.get_tax_fees(e, entry, tr_map, salary)
		#self.get_ntax_bonus_and_tax_bonus(e, entry, tr_map)
		self.get_tax_ot(e, entry, tr_map, salary)
		self.get_tax_commission(e, entry, tr_map, salary)
		self.get_tax_hazard(e, entry, tr_map, salary)
		self.get_other_supp_a(e, entry, tr_map, salary)
		self.get_other_supp_b(e, entry, tr_map, salary)

		#SET TAXABLE COMPENSATION INCOME (REGULAR)
		...
		#SET TAXABLE COMPENSATION INCOME (SUPPLEMENTARY)
	...
	#GET TAXABLE COMPENSATION INCOME (REGULAR)
	def get_tax_bs(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Basic" and is_taxable == 1 and _type == "Income":
					entry['tax_bs'] += d.amount

				if bir_type == "Basic" and is_taxable == 1 and _type == "Deduction":
					entry['tax_bs'] -= d.amount
	...
	def get_tax_rep(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Representation" and is_taxable == 1:
					entry['tax_rep'] += d.amount

		return entry
	
	def get_tax_transpo(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Transportation" and is_taxable == 1:
					entry['tax_transpo'] += d.amount

		return entry

	def get_tax_cola(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "COLA" and is_taxable == 1:
					entry['tax_cola'] += d.amount

		return entry

	def get_tax_housing(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Housing Allowance" and is_taxable == 1:
					entry['tax_housing'] += d.amount

		return entry

	def get_other_reg_a(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Other Regular (A)" and is_taxable == 1:
					entry['other_reg_a'] += d.amount

		return entry

	def get_other_reg_b(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Other Regular (B)" and is_taxable == 1:
					entry['other_reg_b'] += d.amount

		return entry

	#GET TAXABLE COMPENSATION INCOME (SUPPLEMENTARY)
	def get_tax_commission(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Commission" and is_taxable == 1:
					entry['tax_commission'] += d.amount

		return entry

	def get_tax_sharing(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Profit Sharing" and is_taxable == 1:
					entry['tax_sharing'] += d.amount

		return entry

	def get_tax_fees(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Fees" and is_taxable == 1:
					entry['tax_fees'] += d.amount

		return entry

	def get_tax_ot(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Overtime" and is_taxable == 1:
					entry['tax_ot'] += d.amount

		return entry

	def get_tax_hazard(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Hazard" and is_taxable == 1:
					entry['tax_hazard'] += d.amount

		return entry

	def get_other_supp_a(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Other Supplementary (A)" and is_taxable == 1:
					entry['other_supp_a'] += d.amount

		return entry

	def get_other_supp_b(self, e, entry, tr_map, salary):
		bir_type, is_taxable = "", ""
		for d in salary:
			bir_type = tr_map[d.get("pay_code")]['bir_type']
			_type = tr_map[d.get("pay_code")]['type']
			...
			if _type != "None":
				if bir_type == "Other Supplementary (B)" and is_taxable == 1:
					entry['other_supp_b'] += d.amount

		return entry

	#SUMAMRY
	def get_summary(self, e, entry):
		self.get_sum_tcipe(e, entry)
		self.get_sum_td(e, entry)
		self.get_sum_atw_pres(e, entry)
		self.get_sum_atw_prev(e, entry)
	...
	def get_sum_tcipe(self, e, entry):
		prev_bir = frappe.db.sql(""" SELECT DISTINCT * FROM `tabBIR2316` WHERE document_type = "Previous" AND `employee` = %s AND docstatus = 1 """, (self.employee), as_dict=1)
		if prev_bir:
			for d in prev_bir:
	...
	def get_sum_atw_pres(self, e, entry):
		salary = frappe.db.sql(""" SELECT PE.`amount` FROM `tabPayroll Register Entries` PE JOIN `tabPayroll Register` PR ON PE.`parent` = PR.`name` WHERE PE.`pay_code` = "WHTAX" AND PR.on_hold = 0 AND PR.posting_date >= %(from_year)s AND PR.posting_date <= %(to_year)s AND PR.employee = %(employee)s  """,{ 
			"employee": e.name,
			"from_year": self.from_date,
			"to_year": self.to_date,
		}, as_dict=True)
	...
	def get_sum_atw_prev(self, e, entry):
		prev_bir = frappe.db.sql(""" SELECT DISTINCT * FROM `tabBIR2316` WHERE document_type = "Previous" AND `employee` = %s AND docstatus = 1 """, (self.employee), as_dict=1)
		if prev_bir:
			for d in prev_bir:
				entry['sum_atw_prev'] += d.sum_tatwa
	...
	def prev_compute_total_ntax_and_tax(self):
