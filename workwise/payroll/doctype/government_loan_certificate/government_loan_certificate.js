...
frappe.ui.form.on('Government Loan Certificate', {
	refresh: function(frm) {
		if(frm.doc.docstatus == 1){
			frappe.call({
				method: "workwise.setup.doctype.jasper_form.jasper_form.get_forms",
				args:{
					doctype_name: "Government Loan Certificate"
				},
				callback: function(r) {
		...
		cur_frm.set_query("certified_by", function() {
			return {
				"filters": {
					"is_active": 1,
				}
	...
	add: function(frm) {
		if(frm.doc.type && frm.doc.certified_by && frm.doc.sensitivity_level && frm.doc.from_date && frm.doc.to_date){
			frappe.call({
