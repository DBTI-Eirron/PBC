...
def get_employees(filters):
	filt = " AND company = '%s'"%(filters.company)
	if filters.location:
		filt += " AND location = '%s'"%(filters.location)
	if filters.department:
		filt += " AND department	 = '%s'"%(filters.department)

	employees = frappe.db.sql("""SELECT *
	 	FROM tabEmployee
		WHERE on_hold = 0 AND is_active = 1 """+filt+""" ORDER BY last_name, first_name""", as_dict=True)
