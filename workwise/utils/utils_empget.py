...
def empget_employees(filter_type, filter_value, company):
	...
	if filter_value:
		employees = frappe.db.sql("""SELECT {fields} FROM tabEmployee WHERE company = %(company)s {conditions} ORDER BY last_name, first_name""".format(conditions=build_conditions),{ 
			"company": company,
		}, as_dict=1)
