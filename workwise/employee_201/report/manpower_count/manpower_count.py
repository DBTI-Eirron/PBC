...
def get_departments(filters):
	if filters.department:
		return frappe.db.sql("""select name, parent_department, lft, rgt, department_name from `tabDepartment` WHERE name = %s order by lft """,(filters.department), as_dict=True)
	else:
...
def set_emp_entries_by_department(company, as_of_date, root_lft, root_rgt, filters, emp_entries_by_department, ignore_closing_entries=False):
	...
	filt = """ AND company= '%s' and department in (select name from `tabDepartment` where lft >= %d and rgt <= %d)"""%(company,root_lft,root_rgt)
	if filters.location:
		filt += " AND location = '%s'" %(filters.location)

	emp_entries = frappe.db.sql("""select department, job_level as position_title, count(`name`) as qty from `tabEmployee` WHERE 
		is_active = 1
		"""+filt+"""
		group by department, job_level
		order by department """,as_dict=True)

	# emp_entries += frappe.db.sql("""select department, "Target" as position_title, SUM(quantity) as qty from `tabTalent Requisition` WHERE
	# 	docstatus = 1
	# 	"""+filt+"""
	# 	group by department
	# 	order by department """,as_dict=True)
