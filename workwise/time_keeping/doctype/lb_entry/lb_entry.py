...
class LBEntry(Document):
	def validate(self):
		if not self.get('created_from_no_linked_leave_applications'):
	...
	def deduct_add_lbentry_to_overused(self):
		...
		for overused in overused_list:
			if total_add_credit > 0:
				...
				if total_deduction_credit > total_remaining_credit:
					total_deduction_credit = total_remaining_credit
